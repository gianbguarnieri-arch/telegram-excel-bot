...
# ===== Flags =====
DEBUG = (os.getenv("DEBUG", "0").strip() in ("1", "true", "True", "yes", "on"))
...
def msal_token():
    ...
    if missing:
        raise RuntimeError(f"Credenciais MSAL ausentes: {', '.join(missing)}")
...
def _force_date_ddmmyyyy(s: str) -> str:
    """
    Se vier algo tipo 7/10 ou 07-10, devolve DD/MM/AAAA (completa ano atual).
    """
    if not s:
        return s
...
async def telegram_webhook(req: Request):
    try:
        body = await req.json()
        # Log bruto para debug (apaga depois)
        logger.info("Telegram body: %s", body)
        ...
        if text.lower().startswith("/start"):
            await tg_send(chat_id, "Envie algo como: gastei 45,90 no mercado com cartão hoje")
            return {"ok": True}

        # Sempre tentamos interpretar (comando, frase natural, etc.)
        row, err = parse_add(text)
        if err:
            await tg_send(chat_id, f"❗ {err}")
            return {"ok": True}

        # Se DEBUG ligado, manda um resumo do que será lançado
        if DEBUG:
            data_iso, tipo, categoria, descricao, valor, forma, origem = row
            dbg = (f"[DEBUG] Vou lançar:\n"
                   f"Data: {data_iso}\nTipo: {tipo}\nCategoria: {categoria}\n"
                   f"Desc: {descricao}\nValor: R$ {valor:.2f}\nForma: {forma}")
            await tg_send(chat_id, dbg)
